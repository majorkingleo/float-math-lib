...
char buffer[BUFLEN], outbuffer[BUFLEN],errorbuffer[BUFLEN];
char ybuffer[BUFLEN],dbuffer[BUFLEN];

int convert()
 {
//   char buffer[100];     // eingabe
//   char outbuffer[100],
   char expcharbuffer[20];
   ...
   int manvorz, manlen, manstellen, manstellenpkt,/* manstart, manend,*/
       manbuffer[BUFLEN];
   int manstflag, mannullc, mannullv, manpktflag, zeroflag;
   int /*ll,*/ ii, p10;
   /* int A [109][3]; */
/*   printf("-----------------------------------------------------\n");
   printf("                   convert.cpp \n");
...
//   dezppos = 0;
   ...
   if(iok==1)
   {
     sprintf(errorbuffer,">>> Fehlerhafte Eingabe an Position : %d  Zeichen : %c \n",
     errpos+1, buffer[errpos]);
	  return(-1);
   }
   if(ecount > 1)
   {
     sprintf(errorbuffer,">>> Fehlerhafter Exponent, 2 x definiert \n");
	  return(-1);
   }
   if(pluscount > 2 || minuscount > 2)
   {
     sprintf(errorbuffer,">>> Zu viele Vorzeichen, illegale Eingabe \n");
	  return(-1);
   }
...
/*------------------------------------------------------------*/
   if(expfound > -1 ) {
    ...
    //  explen = 0;
      ...
      {
       sprintf(errorbuffer,">>> Illegaler Exponent \n");
       return(-1);
      }
      ii=0;
      for( kk=expstart; kk<=expend  ;kk++) {
           if(buffer[kk] == '.' || buffer[kk] =='+' ||
              buffer[kk] == '-' ) {
             sprintf(errorbuffer,">>> Illegaler Exponent, illegal character \n");
             return (-1);
           }
         if(buffer[kk] == ' ') { continue; }
         // printf("buffer = %c\n",buffer[kk]);
 ...
 /* berechne Stellen bis zum Dezimal Punkt oder bis zum Ende (integer) */
   ...
   for( kk=0; kk<=manlen ;kk++) {
    /*  printf("buffer = %c\n",buffer[kk]); */
     ...
     if(buffer[kk] == '+' && kk > 0) {
        sprintf(errorbuffer,">>> Illegales + an Position :%d\n", kk);
        return(-1);
     }
     if(buffer[kk] == '-' && kk > 0) {
        sprintf(errorbuffer,">>> Illegales - an Position :%d\n", kk);
     ...
     if(buffer[kk] == ' ') {
        sprintf(errorbuffer,">>> Illegales space an Position :%d\n", kk);
        return(-1);
     }
   }
  ...
  // printf("GesamtZahl der vorlaufenden Nullen = %d\n",mannullc);
   zeroflag = 0;
   if(manstellen == 0 && (buffer[0] == '0' || buffer[0]=='+' || buffer[0]=='-')){
    zeroflag = 1;                                // neu
    manstellen = 1; manbuffer[1] = '0';
   }
   if(manstellen == 0 ) {
     sprintf(errorbuffer,">>> Keine Mantisse gefunden\n");
     return(-1);
   }
...
//       printf(" E %3d\n", expsum);
       if(mannullv > 0 ){                               // neu
         if(expvorz == 0) {                              // neu
             expsum = expsumsave - mannullv - 1;        // neu
         }else{                                         // neu
             expsum = -expsumsave - mannullv - 1;        // neu
         }
       }

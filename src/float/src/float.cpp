...
**************************************************************
 ...
 Letzte Aenderung : 013-Oct-98 24:00
                     ...
                     13-Dez-98 ln fertig f�r alle Zahlen
...
/* Die Rechenoperationen werden in folgenden Registern durch-
   ...
   Aufbau der floating point Zahl :
    ...
    Mantisse            : array  0..L�nge, Feld 0 bleibt frei
   ...
   aller Operationen wird immer in dieser Darstellung pr�sentiert.
   Die Eingabe jeder Zahl in die Register muss immer in diesem
   Format erfolgen !!
   Beispiel : 3.88888 e-77  , 1.0 e0,  -9.999 e33
   Beispiel f�r Eingabe : Zahl 1.9e-33
   rx.ma[1]=1; rx.ma[2]=9 ; rx.se = 1 ; rx.ex = 33; rx.sm = 0;

   Beachte, da� addsub die Register rx, ry so vertauscht, dass
   in rx immer der groesste Absolutbetrag steht.
   alle Register m�ssen vom Typ 'wortform' sein.
 */
#include <stdio.h>
#include <stdlib.h>
#include <iostream>
...
/* maximale Werte f�er n = 1030, L�enge = 1050
   L�nge sollte immer um ca 20 gr�sser sein   */
...
/**********************************************************/
/* right shift rb cnt digits, links 0 einf�gen
...
{ int j;
  ...
  do
  {
    ...
    div2:
    ra_plus_rb(); /* Divisor addieren f�r positiv ra */
    leftshift_ra();  /* n�chste Stelle */
    rq.ma[j] = cnt;
    j++;
  } while (!( j > n));
...
//        printf("div teil mult4 1\n");
 ...
 /* �bernimm richtiges Vorzeichen vom Exponent, wenn Differenz 0,
  ...
  /* bei der Division wird unterschieden, ob x >= y (cnta = 0)
     oder x < y. Dann mu� der Exponent noch extra korrigiert werden */
  if(multdiv == true) {  /* div*/
//    printf("div teil mult4 2\n");
  ...
  rightshift_ra(); /* �bertrag, schiebe rechts */
  if(rz.se == 1) {
  /* sign Exponent negativ */
   rz.ex--;
   /* springe noch einmal zur�ck und setze Exp 0 */
   if(rz.ex == 0) {
     goto md2;
   } else {
     move_ra_to_rz(); goto fertig;
...
void divide()
{
  ...
  move_rq_to_ra();
  /* wann kommt die n�chste Zeile vor */
  if(ra.ma[1] == 0){
    leftshift_ra();
  }
...
/* Berechne Wurzel nach der Heron-Methode
   ...
   m2=(zahl/m1 + m1) * 0.5,
   rz = sqrt(rx)
   G�ltiger Bereich f�r rx ist 1 -100
...
void wurzel()
{
 ...
 rm = sieben8;
 rn = rx;
 /* 15 ist ausreichend f�r 100 Stellen, bei groesserer
...
/* berechne arcsin nach der Reihe x + 1*x^3/2*3 +
 ...
 arctan:
  ...
  arctan   arcsin    grad
   ...
   2         0.89     63
   3         0.94     71.5
  Ergebnis : arctan 0.8 bis 1.2 �ber arcsin 0.62 bis 0.76 berechnen mit
   arctan(x) = arcsin(x/sqrt(1+x^2))
   arcsin von 0.8 bis 0.9999 �ber arctan 1.33 bis unendlich berechnen mit
...
*/
 ...
 // 0.99
   ...
   // �berpr�fe auf Vielfaches von 2 pi
   pigauss();
   rx=rz;
...
/**********************************************/
/* Berechnung den nat�rlichen Logarithmus nach einer Reihe
 uv = (u-v)/(u+v)
...
/* Berechne 10^x = 1 +(x*ln10)/1 +(x*ln10)^2/2
  ...
  Reihe berechnet, f�r groessere Werte wird die
...
/**********************************************/
/* Berechnung den nat�rlichen Logarithmus ln10 einer Reihe
...
void piramanujan ()
{
 ...
 rx=null0; rx.ma[1]=4; zahl1d239=rx; // 4
  ...
  piold = null0;
  sum1 = zahl1d57;   // f�r n=0 ist erstezahl = 1103
  // for (i=1; i<= 3; i++) {
  i=0;
  for(;;) {
     i++;
...
/* Definition von Konstanten
  --------------------------   */
// Koeffizienten f�r das Wilkinson Polynom  a0
...
// Koeffizienten f�r das Winson Polynom  a1
...
// Koeffizienten f�r das Winkelson Polynom  a2
...
// Koeffizienten f�r das Winkelson Polynom  a3
...
// Koeffizienten f�r das Winkelson Polynom  a4
...
// Koeffizienten f�r das Winkelson Polynom  a5
...
// Koeffizienten f�r das Winkelson Polynom  a6
...
// Koeffizienten f�r das Winkelson Polynom  a7
...
// Koeffizienten f�r das Winkelson Polynom  a8
...
// Koeffizienten f�r das Winkelson Polynom  a9
 ...
 // Koeffizienten f�r das Winkelson Polynom  a10
 ...
 // Koeffizienten f�r das Winkelson Polynom  a11
 ...
 // Koeffizienten f�r das Winkelson Polynom  a12
 ...
 // Koeffizienten f�r das Winkelson Polynom  a13
 ...
 // Koeffizienten f�r das Winkelson Polynom  a14
 ...
 // Koeffizienten f�r das Winkelson Polynom  a15
 ...
 // Koeffizienten f�r das Winkelson Polynom  a16
 ...
 // Koeffizienten f�r das Winkelson Polynom  a17
 ...
 // Koeffizienten f�r das Winkelson Polynom  a20
  ...
  for(inull=1; inull <=20 ; inull++) {
   ...
   /* for(inull=0;inull<10;inull++) {
    ...
    /* Berechnung der Nullstelle f�er x = 4 nach Newton */
    /* Startwert x=3.5 */
...
// 7.8   Startwert f�r Wurzel nach Heron
...
/*
  ...
  stufige Zahnr�der, Zehner�bertrag �ber Klauen, in Paris vorgef�hrt).
  Arbeiten �ber Zahlenkombinatorik (Pascalsches Dreieck).

  Archimedes und die Kreiszahl PI :
  ---------------------------------
   PI ist der 16. Buchstabe des Griechischen Alphabets und Symbol
   f�r den Umfang des Kreises mit Durchmesser 1.
  ...
  Professor f�r Astronomie, moderne Zahlentheorie, lieferte
  Bahnbrechende Beitr�ge zu fast allen Gebieten der Mathematik.
  Algorithmus zur schnellen Berechnung von PI .

  Heron von Alexandria (2. H�lfte des 1. Jahrhunderts v. Chr.)
  ---------------------
  ...
  "e" ist ein irrationale Zahl, Grundzahl der nat�rlichen Logarithmen.
  ...
  -----------------------------
  Englische Mathematiker, Arbeiten �ber unendliche Reihen.

  Baron John Neper (1550 - 1617)
  ------------------------------
  Schottischer Adeliger, 1614 erste Logarithmentafel erstellt.
  Hat die Bezeichnung Logarithmus eingef�hrt.
